...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT first_name,middle_name,last_name,StudentId FROM student WHERE class= :id order by first_name");
 $stmt->execute(array(':id' => $cid));
 ?><option value="">Select Student </option><?php
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
  ?>
  <option value="<?php echo htmlentities($row['StudentId']); ?>"><?php echo htmlentities($row['first_name']); ?> <?php echo htmlentities($row['middle_name']); ?> <?php echo htmlentities($row['last_name']); ?></option>
  <?php
 }
}
